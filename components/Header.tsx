  ...
  function loginSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    if (isLogin == "login") {
      ...
      if (
        ...
        (e.target as HTMLFormElement).confirm_password.value
      ) {
        instance()
          .post("/register", data)
          .then(() => {
    ...
    <header className="px-[120px] py-[25px]">
      <div className="flex items-center  justify-between">
        ...
        <ul className="flex items-center gap-[50px]">
          <li>
            <Link
              className="font-medium text-[16px] text-[#3D3D3D] border-b-[3px] border-b-transparent hover:border-b-[3px] block hover:text-[#46A358] hover:border-b-[#46A358] duration-300"
              href={"/"}
            >
              Home
            </Link>
        ...
        <div className="flex items-center gap-[30px]">
          ...
          <button className="text-red-500">
            <Badge
              ...
              content={
                token ? (likeProducts.length ? likeProducts.length : "") : ""
              }
            >
              <LikeIcon />
            </Badge>
      ...
      <span className="border-b-[2px] border-b-[#46A35880] w-[1280px] block mt-[25px]"></span>
    </header>
  );
};
